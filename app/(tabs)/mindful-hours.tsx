  ...
  return (
    ...
    >
      ...
      {activeTab === 'timer' ? (
        <TimerView
          colors={colors}
          onCompleted={async (mins) => {
            await withLoading('add-mindful-minutes', async () => {
              await addMindfulMinutes({ seconds: mins * 60, date_iso: new Date().toISOString() });
            });
          }}
        />
      ) : (
        <HistoryView
          ...
          history={history.map((e) => ({
            date: e.date_iso,
            minutes: Math.round((e.seconds ?? 0) / 60),
          }))}
          loading={loading}
        />
      )}
...
function TimerView({
  ...
  }, [isActive, pulseAnim, secondsRemaining, initialSeconds, onCompleted]);
...
function HistoryView({
  ...
  if (loading) {
    return (
      <View style={{ gap: 12 }}>
        <SkeletonRect height={70} borderRadius={UI.radius.lg} />
        <SkeletonRect height={70} borderRadius={UI.radius.lg} />
        <SkeletonRect height={70} borderRadius={UI.radius.lg} />
        <SkeletonRect height={70} borderRadius={UI.radius.lg} />
      </View>
